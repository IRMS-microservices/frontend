...
export function Sidebar({ role }: AppSidebarProps) {
  ...
  return (
    <aside className="w-55 shrink-0 bg-irms-green flex flex-col h-full">
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg mb-1 text-xs font-semibold tracking-wider transition-all duration-150 ${isActive
